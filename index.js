(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["filterByPinyin"] = factory();
	else
		root["filterByPinyin"] = factory();
...
var filter = function filter(input, option) {
  ...
  for (var i = 0; i < arr.length; i++) {
    ...
    var subList = list.slice(i);
    var subArr = arr.slice(i);

    var result = check(input, subList, subArr);

    var inputWithoutPinyinSplit = input.replace(/'/g, '');
...
var check = function check(input, list, arr) {
  if (!input.length) return true;
  if (input.length && !arr.length) return false;
  for (var i = 1; i <= input.length; i++) {
    var subInput = input.slice(0, i);
    if (match(subInput, list[0], arr[0]) === false) {
      return false;
    }
    var result = check(input.slice(i), list.slice(1), arr.slice(1));
    if (result) return true;
  }
  return false;
};

var match = function match(subInput, listItem, arrItem) {
  ...
  try {
    for (var _iterator = arrItem[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
...
var pinyin = function pinyin(str) {
  var reg = /[\u4E00-\u9FA5]/;
  var match = str.match(reg);
  if (!match) return [str];
  var char = match[0];
  var retList = [];
  for (var key in _dict2.default) {
    if (_dict2.default[key].indexOf(char) >= 0) {
      retList.push(key);
    }
  }
